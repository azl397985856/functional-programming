...
## 总结
这一节我们介绍了如何在函数式中使用容器处理分支逻辑，如何处理异步情况，以及如何加二者的嵌套结合起来。
对于不同的地方我们用到了不同的容器，函数式世界中有很多很多容器。

最后通过一个例子，让大家更直观地感受到函数式编程对逻辑的抽象，使得代码天生复用性，可读性都很强。
不过理解和熟练使用会比较困难， 还是那句话，重要的是改变思路，用函数式思维去编程。
